...
Evaluating the gradient in any point $\boldsymbol{\theta} \in \Theta$ returns a vector in the parameter space $\Theta$ that points in the direction that results in the steepest ascent in $\mathcal{L}$.
...
\]
Hence, taking small steps in the direction of the negative gradient will lead to a local minimum or a saddle point.
...
A necessary prerequisite for gradient descent is that the loss function is differentiable.
This also depends on what kind of hypothesis functions are considered.
Moreover, for non-convex functions convergence of the algorithm is not guaranteed.
In theory, gradient descent can converge in saddle points or undesirable local minima.

However, in praxis, the method has proven to reliably find low values in the loss function in reasonable time.
...
%large variety of modifications to gradient descent method
%gradient accumulation / \enquote{Nesterov} (momentum)
%methods with second derivative (\enquote{second order} )
%
%
%computing exact gradient is slow
%take a random subset of data points and calculate gradient of partial loss
%partial gradient is unbiased estimate of real gradient
%learnrate needs to be gradually decreased
%sgd asymptotically following gradient of generalization error
%additional benefits: less likely to overfit, better stability, regularizing effect
