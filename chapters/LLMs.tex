...
% outut a probability distribution
% next token preidction -> how likely is each token to appear
% distribtuion over all possible tokens in the vocabulary $V$
% prediction function has to output discrete distribution over tokens
% as prediction function is estimated by neural network -> neural network with |V| output neurons
% single neuron represents degree of beflief for the occurance of a specific token
% output \textit{logits} not really interpretable as distributions

%Under the condition of an inputs sequence of words $x$ the di
...
% cross-entropy -> principle of maximum likelihood
