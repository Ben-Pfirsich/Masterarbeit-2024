...
\[
h^* = \underset{{h \in \mathcal{H}}}{\mathrm{argmin}} \, \mathbb{E}[L(h(X^*), Y^*)]
\]
...
\[
    \hat{h} = \underset{{h \in \mathcal{H}}}{\mathrm{argmin}} \, \frac{1}{m}\sum_{i=1}^{m} L(h(x_i), y_i)
\]
...
By setting aside a small subset of the training data as \textit{test data} which is not used in the optimization process, it is possible to estimate generalization error.
