...
A typical application for \gls{ml} is to learn some task that can be represented by a mapping:
\[
...
%How these sets look specifically depends on the type of task that is learned.

\gls{ml} can be applied to many different tasks like \textit{classification} and \textit{regression}.
For classification, the desired function assigns an integer representing a category to each possible input example.
An example for this is the categorization of images.
...
%\gls{ml} makes it possible to find a classification function $f:\mathbb{R}^n \rightarrow \{1,\dots,k\}$ that assigns an integer representing a category to each possible input vector.
