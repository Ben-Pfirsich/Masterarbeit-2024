...
In the 1990s, \glsfirst{slm} found widespread use.
The models often assume that the distribution of a word only depends on a fixed length of previous words.
\enquote{$n$-grams models} are \glsentryshort{slm}s that use the previous $n$ words to calculate the probability distribution of the next word.
...
\]
In recent years, research has been focused on the more flexible \glsfirst{nlm}~\autocite{quadarLM2020}.
This approach uses \glsfirst{dnn} to model the probability distributions over words.
